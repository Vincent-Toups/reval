#' Filter rows where all specified columns have NA values
#' 
#' This function filters out the rows in a data frame where all values in the specified columns are NA.
#' 
#' @param df A data frame containing the data to be filtered.
#' @param cols A character vector of column names that should be checked for NA values.
#' 
#' @return A data frame with rows removed where all specified columns are NA.
#' 
#' @examples
...
#' )
#' 
#' # Filter out rows where all of columns 'a', 'b', and 'c' are NA
#' filtered_df <- filter_rows_all_na(df, c("a", "b", "c"))
#' 
#' @export
filter_rows_all_na <- function(df, cols) {
  sum_na <- rowSums(is.na(df[, cols]))
  num_cols <- length(cols)
  df[sum_na != num_cols, ]
}


#' Package-level variable for error message context
#'
#' This is a package level variable that provides extra context
#' for error messages generated by this library. It is intended
#' to be temporarily set by `with_error_context()` function to
...
#' Execute a block of code with additional error context
#'
#' This function sets a new error context, executes a given block of code,
#' and then restores the previous error context. This is useful for providing
#' more detailed error messages.
#'
#' @param new_error_context The new context message to set.
#' @param body The block of code to execute with the new error context.
#' @param ... Additional arguments passed to the body function.
#'
#' @return The value returned by the body function.
#'
#' @examples
#' \dontrun{
#' with_error_context("Reading data file", {
#'   # Your code here e.g., read.csv("some_file.csv")
#' })
#' }
#'
#' @export
with_error_context <- function(new_error_context, body, ...){
    old_error_context <- error_context
    error_context <<- new_error_context
    x <- body(...)
    error_context <<- old_error_context
    x
}
...
convert_yes_no_to_logical <- function(char_vec) {
  ...
  if(any(is.na(char_vec))){
    char_vec[is.na(char_vec)] <- "Yes"
    warning("NA values have been replaced with Yes")
  }
...
get_variable_type <- function(dataset_name, variable_name, spec){
    ...
    if (variable_name == "QSEVLINT") { 
        # Special case for QSEVLINT
        "QSEVLINT"
    } else if (variable_name == "FTSTAT") {
        # Special case for FTSTAT
        "FTSTAT"
...
parses_as_formatted_float <- function(input_string, format) {
    ...
    if(length(format_parts)==1){
        format_parts <- c(format_parts, "0");
    }
...
check_qsevlint <- function(dataset, combined_spec) {
  ...
  spec_subset <- combined_spec %>% 
    ...
    df <- dataset %>%
        dplyr::inner_join(spec_subset, by=c("QSTESTCD"="codelist_terms")) %>%
        dplyr::filter(QSEVLINT!=evaluation_interval);
    
    indices <- df %>%
        dplyr::filter(QSEVLINT!=evaluation_interval) %>% dplyr::pull(index__);

    if(length(indices) == 0){
        tibble::tibble(check_name="QSEVLINT must match its QSTESTCD.",pass=T, message="All matched.", row_numbers=NA);
    } else {
        tibble::tibble(check_name="QSEVLINT must match its QSTESTCD.",pass=F, message=sprintf("%d rows failed to match spec.",length(indices)), row_numbers=format_row_numbers(indices));
    } 
}
...
find_name_pairs <- function(names) {
    # Filter out names that start with "unparsed__"
    parsed_names <- grep("^unparsed__", names, value = TRUE, invert = TRUE)
    
    # Extract base names by removing the last character
    base_names <- substr(parsed_names, 1, nchar(parsed_names) - 1)
    ...
    name_pairs <- lapply(dup_base_names, function(base_name) {
        pair <- parsed_names[base_names == base_name]
        return(pair)
    })
...
validate_dataset <- function(filename, combined_specs=function(){reval::min_data_set_spec}, write_report_to=FALSE){
    ...
    if (is.character(write_report_to) && write_report_to != FALSE) {
    ...
    if(all(c("FTSTRESC", "FTSTRESN", "FTSTAT") %in% names(data))){
        messages <- check_missing_values_in_ft(data);
    ...
    for(v in variables){
        ...
        messages <- rbind(messages, switch(var_type,
               ...
               FTSTAT = rbind(check_ftstat_values(data),check_missing_values_in_ft(data)),
...
if (identical(write_report_to, stdout)) {
        # Print to stdout
        if(nrow(messages) > 0) {
            messages %>% dplyr::filter(pass==FALSE) %>%
                dplyr::transmute(output = paste0(message, " Rows: ", row_numbers)) %>% 
                dplyr::pull(output) %>% 
                cat(sep="\n\n")
        }
    } else if(is.character(write_report_to)){
        # Write to a file
        switch(tools::file_ext(write_report_to),
               ...
               txt={
                   messages %>% dplyr::filter(pass==FALSE) %>%
                       dplyr::transmute(output = paste0(message, "\n Rows: ", row_numbers)) %>% 
                   dplyr::pull(output) %>%
                       cat(file=write_report_to, sep="\n\n");
               })
    }
    messages
}
...
#' Check that FTSTRESC, FTSTRESN, and FTSTAT are not all missing.
#'
#' @param ft_data The data frame containing the ft dataset.
#' @return A tibble summarizing the results of the check.
check_missing_values_in_ft <- function(ft_data) {
  
  # Identify rows where all specified columns are missing
  missing_data <- ft_data %>% 
    dplyr::filter(is.na(FTSTRESC) & is.na(FTSTRESN) & is.na(FTSTAT))
  
  indices <- missing_data %>% dplyr::pull(index__)  # Replace `index__` with the actual name of your index column if different
  
  if (length(indices) == 0) {
    return(tibble::tibble(
      check_name = "FTSTRESC, FTSTRESN, and FTSTAT should not all be missing.",
      pass = TRUE,
    ...
    return(tibble::tibble(
      check_name = "FTSTRESC, FTSTRESN, and FTSTAT should not all be missing.",
      pass = FALSE,
      message = sprintf("%d rows are missing all three of FTSTRESC, FTSTRESN and FTSTAT - these rows should be deleted.", length(indices)),
      row_numbers = format_row_numbers(indices)  # Assuming `format_row_numbers` is a function you've defined
    ))
  }
}

#' Check that FTSTAT, where it does exist, is "Not done".
#'
#' @param ft_data The data frame containing the ft dataset.
#' @return A tibble summarizing the results of the check.
check_ftstat_values <- function(ft_data) {
  
  # Identify rows where FTSTAT exists but is not "Not done"
  incorrect_values <- ft_data %>%
    dplyr::filter(!is.na(FTSTAT) & FTSTAT != "Not done")
  
  indices <- incorrect_values %>% dplyr::pull(index__)
  
  if (length(indices) == 0) {
    return(tibble::tibble(
      check_name = "FTSTAT, where it does exist, should be 'Not done'.",
      pass = TRUE,
      message = "All rows are valid.",
      row_numbers = NA
    ))
    ...
    return(tibble::tibble(
      check_name = "FTSTAT, where it does exist, should be 'Not done'.",
      pass = FALSE,
      message = sprintf("%d rows have incorrect FTSTAT values.", length(indices)),
      row_numbers = format_row_numbers(indices)
    ))
  }
}

#' Check for rows where both columns in each name pair are missing and create a boolean array.
#'
#' This function identifies pairs of columns that differ only by their last character.
#' It then checks if both columns in the pair are missing for the same row.
#' A summary of the check is returned in a tibble.
#' Pairs of columns where either name starts with 'FT' are ignored.
#' A boolean array where every index in problematic_rows is 0 and all other indices are 1 is computed.
#'
#' @param data The data frame to check.
#' @return A list containing a tibble summarizing the results of the check and a boolean array.
check_missing_values_in_pairs <- function(data) {
  
  # Identify pairs of variable names that differ only by their last character
  name_pairs <- find_name_pairs(names(data))
  
  # Initialize results list and list to hold problematic row indices
  results <- list()
  problematic_rows <- c()
  ...
  for(i in 1:length(name_pairs)) {
    ...
    # Skip pairs where either name starts with 'FT'
    if(startsWith(pair[1], 'FT') || startsWith(pair[2], 'FT')) {
      next
    }
    
    # Identify rows where both columns in the pair are missing
    missing_data <- dplyr::filter(data, is.na(.data[[pair[1]]]) & is.na(.data[[pair[2]]]))
    
    indices <- missing_data %>% dplyr::pull(index__);
    
    # Record problematic rows
    if(length(indices) > 0) {
      problematic_rows <- c(problematic_rows, indices)
    }
    
    # Create a summary tibble
    if (length(indices) == 0) {
      results[[i]] <- dplyr::tibble(
        check_name = paste0("Columns ", pair[1], " and ", pair[2], " should not both be missing."),
        pass = TRUE,
        message = "All rows are valid.",
        row_numbers = NA
      )
      ...
      results[[i]] <- dplyr::tibble(
        check_name = paste0("Columns ", pair[1], " and ", pair[2], " should not both be missing."),
        pass = FALSE,
        message = sprintf("%d rows are missing both columns %s and %s - these rows should be deleted.", length(indices), pair[1], pair[2]),
        row_numbers = format_row_numbers(indices)
      );
    }
  }
  
  # Combine all result tibbles into one
  result_tibble <- dplyr::bind_rows(results)
  
  # Create boolean array
  boolean_array <- rep(TRUE, nrow(data));
  boolean_array[problematic_rows] <- FALSE;
  
  return(list(summary = result_tibble, problematic_rows = boolean_array))
}
